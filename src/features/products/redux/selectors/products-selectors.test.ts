...
describe('products selectors', () => {
    ...
    describe('selectProductsListIsLoading', () => {
        it('should return isLoading', () => {
            expect(
                sut.selectProductsListIsLoading({
                    products: {
                        list: {
                            isLoading: true,
                            items: []
                ...
                } as Partial<SandboxState> as SandboxState)
            ).toEqual(true);
        });
    }); // Закрывающая скобка для второго describe

}); // Закрывающая скобка для основного describe
